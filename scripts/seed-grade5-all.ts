...
async function seedGrade5All() {
  console.log('🌱 Starting Grade 5 All Subjects seeding...\n');

  try {
    // Define all curriculum files for Grade 5
    const curriculumFiles = [
      { path: 'fixtures/curriculum/grade5-2025-2026-math.json', subject: 'Toán' },
      { path: 'fixtures/curriculum/grade5-2025-2026-english.json', subject: 'Tiếng Anh' },
      { path: 'fixtures/curriculum/grade5-2025-2026-science.json', subject: 'Khoa học' },
      { path: 'fixtures/curriculum/grade5-2025-2026-literature.json', subject: 'Tiếng Việt' },
    ];

    // Define all exercise files for Grade 5
    const exerciseFiles = [
      // Math
      { path: 'fixtures/math/grade5-2025-math-easy.json', difficulty: 'easy', subject: 'Toán' },
      { path: 'fixtures/math/grade5-2025-math-medium.json', difficulty: 'medium', subject: 'Toán' },
      { path: 'fixtures/math/grade5-2025-math-hard.json', difficulty: 'hard', subject: 'Toán' },
      // English
      { path: 'fixtures/english/grade5-2025-english-easy.json', difficulty: 'easy', subject: 'Tiếng Anh' },
      { path: 'fixtures/english/grade5-2025-english-medium.json', difficulty: 'medium', subject: 'Tiếng Anh' },
      { path: 'fixtures/english/grade5-2025-english-hard.json', difficulty: 'hard', subject: 'Tiếng Anh' },
      // Science
      { path: 'fixtures/science/grade5-2025-science-easy.json', difficulty: 'easy', subject: 'Khoa học' },
      { path: 'fixtures/science/grade5-2025-science-medium.json', difficulty: 'medium', subject: 'Khoa học' },
      { path: 'fixtures/science/grade5-2025-science-hard.json', difficulty: 'hard', subject: 'Khoa học' },
      // Literature
      { path: 'fixtures/literature/grade5-2025-literature-easy.json', difficulty: 'easy', subject: 'Tiếng Việt' },
      { path: 'fixtures/literature/grade5-2025-literature-medium.json', difficulty: 'medium', subject: 'Tiếng Việt' },
      { path: 'fixtures/literature/grade5-2025-literature-hard.json', difficulty: 'hard', subject: 'Tiếng Việt' },
    ];
    ...
    for (const file of curriculumFiles) {
      if (fs.existsSync(path.join(process.cwd(), file.path))) {
        console.log(`📖 Processing ${file.subject}...`);
    ...
    // Step 2: Create Subject/Chapter/Section/Lesson structures and seed exercises
    console.log('\n📚 Step 2: Creating structures and seeding exercises...\n');
    
    // Group exercises by subject
    const exercisesBySubject = new Map<string, typeof exerciseFiles>();
    for (const ex of exerciseFiles) {
      if (!exercisesBySubject.has(ex.subject)) {
        exercisesBySubject.set(ex.subject, []);
      }
      exercisesBySubject.get(ex.subject)!.push(ex);
    }

    // Process each subject
    for (const [subject, exercises] of exercisesBySubject.entries()) {
      // Find curriculum file for this subject
      const curriculumFile = curriculumFiles.find(f => f.subject === subject);
    ...
    console.log('\n✅ Grade 5 All Subjects seeding completed successfully!');
  } catch (error) {
